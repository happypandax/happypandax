...
import StatusCardLabel, {
  statusCardLabelDataFields,
...
import { ServerStatus } from '../../misc/types';
...
export async function getServerSideProps(context: NextPageContext) {
  ...
  const data = await server.search_items<ServerStatus>({
    item_type: ItemType.Status,
    search_query: urlQuery.query?.q?.toString() as string,
    fields: statusCardLabelDataFields,
    limit,
    offset: (page - 1) * limit,
    sort_options: {
      by: ItemSort.StatusName,
    },
  });
    ...
    <DirectoryPage>
      <Container as={Segment} centered clearing basic>
        <Statistic horizontal color="grey">
          ...
          <Statistic.Label>{t`Status`}</Statistic.Label>
        </Statistic>
        <ItemSearch
          stateKey="status"
        ...
        <Grid doubling centered stackable columns="3">
          {data.items.map((i: ServerStatus) => (
            <Grid.Column key={i.id}>
              <StatusCardLabel data={i} centered fluid />
            </Grid.Column>
          ))}
        </Grid>
      </PaginatedView>

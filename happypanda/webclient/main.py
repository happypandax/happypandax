...
def run(blocking=True):
    try:
        log.i("Webserver successfully starting... ({}:{}) {}".format(constants.host_web, constants.port_webserver, "(blocking)" if blocking else ""), stdout=True)
        # OBS: will trigger a harmless socket.error when debug=True (stuff still works)
        socketio.run(happyweb, *utils.connection_params(web=True), block=blocking, debug=constants.dev)
    except OSError as e:

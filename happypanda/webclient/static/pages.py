...
class BasePage(Base):
    ...
    def get_versions(self, data=None, error=None):
        ...
        if data and not error:
            ...
            vs.append(('torrent client', ".".join(str(x)
                                                  for x in data['torrent'])))
        elif error:
            pass
        else:
...
class ApiPage(Base):
    ...
    def call(self):
        ...
        def each_d(index, element):
            ...
            if key and value:
                ...
                if value.startswith('[') and value.endswith(']'):
                    value = [
                        x.strip() for x in value.replace(
                            '[', '').replace(
                            ']', '').split(',') if x]
                func_args[key] = value
        ...
        client.call([f_dict], lambda msg: S(
...
def init():
    ...
    window.addEventListener(
        'resize',
        lambda: library.grid.mount() if library.grid else None)

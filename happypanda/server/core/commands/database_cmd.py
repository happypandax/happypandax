...
class GetModelByID(Command):
    ...
    def main(self, model: db.Base, ids: set, limit: int = 999, filter: str = "", order_by: str = "", offset: int = 0) -> tuple:
        ...
        if len(ids) > _max_variables:
            ...
            while len(ids_list):
                queries.append(q.filter(model.id.in_(ids_list[:_max_variables])))

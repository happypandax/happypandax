...
HPX can be translated and localized to your language.
...
It is required that a language namespace named ``general`` exists with this *translation id* for every translation language:
...
I'm probably going to create a tool (or you can too) for helping translating later if I have time.

Developer's perspective
-------------------------------------

Developers can choose to create their own language namespace for their own clients if they desire.
This language namespace can also be included in future HPX releases. Just create a pull request.

Say I made an app client for HPX. For this app I could create language files with the namespace ``myapp`` and include them in HPX.
...
For example: ``"general.locale"`` -> ``English (United States)``. 
...
Keep in mind that the locale settings for HPX are *client-scoped*, meaning that you can change the locale settings freely without interfering with other client's locale settings.

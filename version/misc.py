...
class BaseViewMenu(QMenu):
    delete_items = pyqtSignal(bool)
    edit_item = pyqtSignal(object, object)

    def __init__(self, view, index, app_window, selected_indexes=None):
        super().__init__(app_window)
        self.parent_widget = app_window
        self.view = view
        self.index = index
        self.item = index.data(app_constants.ITEM_ROLE)
        self.selected = selected_indexes
        self.item_type = index.data(app_constants.QITEM_ROLE).type()
        self.is_collection = self.item_type == app_constants.COLLECTION_TYPE
        self.is_gallery = self.item_type == app_constants.GALLERY_TYPE
        self.is_page = self.item_type == app_constants.PAGE_TYPE
        
        if self.is_gallery and self.view.view_type == app_constants.ViewType.Default:
            if not self.selected:
                ...
                favourite_act = self.addAction('Favorite selected')
                favourite_act.setCheckable(True)
                #f = []
        ...
        self.addSeparator()
        if not self.selected:
            get_metadata = self.addAction('Add to metadata queue')
        else:
            #gals = []
            #for idx in self.selected:
            #    gals.append(idx.data(Qt.UserRole + 1))
            get_select_metadata = self.addAction('Add selected to metadata queue')
        self.addSeparator()
        ...
        #                        lambda: self.delete_items.emit(False))
        ...
        #else:
        #    remove_select_g = remove_menu.addAction('Remove selected', lambda: self.delete_items.emit(False))
        #remove_menu.addSeparator()
        #if not self.selected:
        #    remove_source_g = remove_menu.addAction('Remove and delete files',
        #                               lambda: self.delete_items.emit(True))
        #else:
        ...
        #else:
        #    self.allow_metadata_exed = False if not self.item.exed else True
    ...
    def allow_metadata_fetch(self):
        ...
        if self.selected:
            ...
            self.item.exed = exed
            gallerydb.execute(gallerydb.GalleryDB.modify_gallery, True, self.item.id, {'exed':exed})

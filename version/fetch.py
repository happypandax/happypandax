...
class Fetch(QObject):
    ...
    def fetch_metadata(self, gallery=None, hen=None, proc=False):
        ...
        if not metadata: # metadata fetching failed
            if gallery:
                ...
                log_i("An error occured while fetching metadata with gallery: {}".format(gallery.title.encode(errors='ignore')))
            return None
    ...
    def _auto_metadata_process(self, galleries, hen, valid_url, **kwargs):
        ...
        for x, gallery in enumerate(galleries, 1):
            ...
            self.AUTO_METADATA_PROGRESS.emit("({}/{}) Adding to queue: {}".format(x, len(galleries), gallery.title))
        ...
        if checked_pre_url_galleries:
            for x, gallery in enumerate(checked_pre_url_galleries, 1):
                self.AUTO_METADATA_PROGRESS.emit("({}/{}) Adding to queue: {}".format(x, len(checked_pre_url_galleries), gallery.title))
            ...
            for x, g_data in enumerate(multiple_hit_galleries, 1):
                ...
                if not gallery.link:
                    ...
                    if isinstance(hen, (pewnet.EHen, pewnet.ExHen)):
                        self.GALLERY_EMITTER.emit(gallery, None, None)
                gallery.temp_url = url
                self.AUTO_METADATA_PROGRESS.emit("({}/{}) Adding to queue: {}".format(x, len(multiple_hit_galleries), gallery.title))

...
def _add_to_db(galleries):
    sess = db_constants.SESSION()
    if isinstance(galleries, list):
        sess.add_all(galleries)
    else:
        sess.add(galleries)
    sess.commit()

class Executors:
    _general_exec = futures.ThreadPoolExecutor()
    ...
    @classmethod
    def add_gallery(cls, galleries):
        if galleries:
            log_d("Adding gallery to db")
            f = cls._general_exec.submit(_add_to_db, galleries)
            return f